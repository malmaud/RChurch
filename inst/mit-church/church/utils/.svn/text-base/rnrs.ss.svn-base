...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
; Drop-in replacement for (rnrs base).
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(library (church utils rnrs)
  ...
  * + - / < <= = > >= abs acos angle append apply asin atan
  ...
  vector-set! vector-ref vector? zero?
 ...
 ;current-output-port                    ; would be duplicated below
  ...
  ; the following imports are added from srfis:
  ; string-hash string-ci-hash
  ; vector-map vector-for-each vector->list list->vector vector-fill! vector-length 
  )

 ;; Add all variables you want to redefine to the exceptions
 ;; listed in the (except (rnrs) ...) clause.

 (import (except (rnrs) 
            ; the following imports are added from AD:
            real? negative? positive? zero? >= <= > < = atan cos sin expt log exp sqrt / * - +
            ; the following are added from xitomatl's profiler
