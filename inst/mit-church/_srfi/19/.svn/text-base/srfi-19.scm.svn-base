...
;;
;; Modified by Derick Eddington to be included into the (_srfi :19 time) R6RS library.
    ...
    [(assoc type tm:time-error-types)
     ...
     (lambda (p)
       (if value
         ...
         (error caller (cdr p))))]
    [else
     (error caller "(library (_srfi :19 time)) internal error: unsupported error type" type)]))
